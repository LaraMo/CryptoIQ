...
class GameGenerator {
  ...
  strategies = {};
  strategy;
  MAX_GENERATION_ATTEMPT = 5;

  constructor(data) {
    const {general, vocalbulary} = data;
  ...
  loadStrategies() {
    ...
    const _2GPN = new TwoGamesWithPageNumberStrategy('2GPN', this.vocabulary);
    const _2G = new TwoGameNoBookStrategy('2G', this.vocabulary);
    const _3GLPN = new ThreeGamesWithLockAndPageNumberStrategy(
      '3GLPN',
      this.vocabulary,
    );
    const _3GPN = new ThreeGamesWithBooks('3GPN', this.vocabulary);
    const _3G = new ThreeGamesNoBookStrategy('3G', this.vocabulary);
    const _4GLPN = new FourGamesWithLockAndBooks('4GLPN', this.vocabulary);
    const _4GPN = new FourGamesWithBooks('4GPN', this.vocabulary);
    ...
    try {
      ...
      switch (this.storyline.difficultyLevel) {
        case Difficulty.EASY.VALUE:
          if (this.generalInfo.textbook) {
            ...
            else generated = await this.strategies['2GPN'].generate();
          } else {
        ...
        case Difficulty.MEDIUM.VALUE:
          if (this.generalInfo.textbook) {
            ...
            else generated = await this.strategies['3GPN'].generate();
          } else {
        ...
        case Difficulty.ADVANCED.VALUE:
          ...
          else generated = await this.strategies['4GPN'].generate();
          break;
        default:
      ...
      if (this.generateTicket && this.ticketMessage) {
        ...
        for (let i = 0; i < noTicket; i++) {
          tickets.push();
          this.insPdf.push(
            await new TicketGenerator(this.ticketMessage).toInstructionPdf(),
          );
        }
      }
    } catch (e) {
      ...
      this.gamePdf = [];
      this.insPdf = [];
      this.MAX_GENERATION_ATTEMPT--;
      if (this.MAX_GENERATION_ATTEMPT !== 0) {
        await this.generate();
      } else {
        throw {
          name: 'GameGenerationError',
          level: 'Show Stopper',
          message:
            "Can't generate game! Please add more words to the vocabulary pool and retry again!",
          htmlMessage:
            "Can't generate game! Please add more words to the vocabulary pool and retry again!",
          toString: function() {
  ...
  async pushHeader() {
    ...
    title.forEach(part => this.insPdf.push(part));
    ...
    let teamSizeRecommendedText = `${this.teams[0]} teams - ${Object.keys(
      teamSize,
    )
      .map(
        val =>
          `${teamSize[val]} of ${val > 1 ? val + ' people' : val + ' person'}`,
      )
      .join(', ')}`;
    ...
    meta.forEach(m => this.insPdf.push(m));
  ...
  pushStoryLine(storylineEnum, pdfArray) {
    ...
    if (pdfArray) {
      pdfArray.push(
        textInstruction(this.storyline[storylineEnum.toLowerCase()], options),
      );
      pdfArray.push({
        type: PdfObjectType.BR,
      });
  ...
  pushPuzzle(puzzle, pdfArray) {
    ...
    } else console.error('Invalid input for puzzle generated');
  }
