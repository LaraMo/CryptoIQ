...
class Crossword {
    ...
    genreateQuestionIns(wordList, withAnswer=false) {
        let pdfIns = [];
        for (let i = 0; i < wordList.length; i++) {
            let answer = wordList[i].word;
            let question = this._clues.filter(val => val.answer === answer)[0].question;
            pdfIns.push({
    ...
    generateGuideIns() {
        ...
        pdfIns = [...pdfIns, ...this.genreateQuestionIns(this.horizontalClues)]
        pdfIns = [...pdfIns, {
            type: PdfObjectType.TEXT,
            text: "Down"
        }, ...this.genreateQuestionIns(this.verticalClues)]
        return pdfIns;
    } 
    ...
    toInstructionPdf() {
        let pdfIns = [{
            ...
            },
            {
                ...
                callback: async (doc) => {
                    console.log("In cb")
                    const imageData = drawGrid(this.board, true, true, true);
                    doc.image(imageData);
                }
            },
    ...
    toGamePdf() {
        let pdfIns = [{
            ...
            },
            {
                ...
                callback: async (doc) => {
                    console.log("In cb")
                    const imageData = drawGrid(this.board, false, true, true);
                    doc.image(imageData);
                }
            },
            ...this.generateGuideIns()
